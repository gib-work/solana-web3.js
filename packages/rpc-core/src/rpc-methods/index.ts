...
export function createSolanaRpcApi(config?: Config): IRpcApi<SolanaRpcMethods> {
    ...
    const patchParameters = <T>(rawParams: T, methodName: string) =>
        patchParamsForSolanaLabsRpc(
            rawParams,
            handleIntegerOverflow ? (keyPath, value) => handleIntegerOverflow(methodName, keyPath, value) : undefined
        ) as unknown[];
    const patchResponse = <T>(rawResponse: unknown, methodName: string): T =>
